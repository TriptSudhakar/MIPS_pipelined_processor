...
struct MIPS_Architecture
{
    ...
    vector<string> RR(vector<string> input)
    {
        if(input[0]=="addi")
        {
            string r1 = input[1];
    ...
    vector<string> RRb(vector<string> input)
    {
        string r1 = input[1];
        string r2 = input[2];
    ...
    vector<string> RRi(vector<string> input)
    {
        ...
        if(input[0] == "sw")
        {
            input[1] = to_string(registers[registerMap[input[1]]]);
        }
        else
        {
            locks[registerMap[input[1]]] += 1;
            temp_locks[registerMap[input[1]]] += 1;
        }
        return input;
    }
        ...
        while(true)
        {
            ...
            if(!stall7)
            {
                ...
                if(rjb[0][0]!="khaali")
                {
                    ...
                    {
                        int r1val = str_to_int(rjb[0][1]);
                        int r2val = str_to_int(rjb[0][2]);
            ...
            if(init[4][0]!="khaali")
            {
                if(init[4][0] == "lw" || init[4][0] == "sw")
                {
                    ...
                    if(!stall9)
                    {
                        ...
                        {
                            if(!locks[registerMap[init[4][1]]] && !locks[registerMap[init[4][2]]])
                            {
                                vector<string> aluinput;
                                aluinput = RRi(init[4]);
                                newls[0] = aluinput;
                ...
                else
                {
                    ...
                    {
                        stall57 = false;
                        if(init[4][0]=="j") newrjb[0] = init[4];
                        else if(init[4][0] == "beq" || init[4][0] == "bne")
                        {
                            if(!locks[registerMap[init[4][1]]] && !locks[registerMap[init[4][2]]])
                            {
                                vector<string> aluinput;
                        ...
                        {
                            if(!locks[registerMap[init[4][2]]] && !locks[registerMap[init[4][3]]])
                            {
                                vector<string> aluinput;
                                aluinput = RR(init[4]);
                                newrjb[0] = aluinput;
